...
import {MapCard} from '@components/shared/MapSelect';
  ...
  render() {
    return (
      <Dialog fullScreen open={this.props.open} onClose={this.props.handleClose}>
        <Grid container justify="space-between" alignItems="flex-start" style={{flexDirection: "row-reverse"}}>
          <IconButton
            margin-left="auto"
            // edge="start"
            color="inherit"
          ...
          {this.props.map ? (
            <Box mt={3} style={{maxWidth: "85%", margin: "10px auto 10px 20px"}}>
        ...
        />{/* Placeholder */}
        {this.props.map ? (
        <TableContainer component={Card} raised style={{maxWidth: "90%", margin: "10px auto", maxHeight: "150px", overflow: "scroll"}}>
